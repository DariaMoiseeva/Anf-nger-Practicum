...
# Create a new dataframe,only include the columns we need
newdf <- data.frame(challenge= merged_df2$challenge,per =merged_df2$per)
...
# Create a line plot with unique lines for each challenge and dodge by industry
ggplot(merged_df2) +
  geom_line(mapping = aes(x = industry, y = per, group = interaction(challenge, industry), color = challenge),
            position = position_dodge(width = 0.5)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(legend.text = element_text(size = 6)) +
  labs(x = "Industry", y = "Percentage of challenges") +
...
ggplot(distinct_df,mapping = aes(x=orderchallenge, y=percentage,fill=orderchallenge))+
  ...
  guides(fill = FALSE)+
   theme(axis.text.x = element_text(angle = 45, hjust = 1))+
   theme(legend.text = element_text(size = 6))+
  labs(x = "challenge", y = "mean  percentage of each challenge ")
